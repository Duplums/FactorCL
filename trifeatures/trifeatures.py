...
class TrifeaturesDataModule(LightningDataModule):
    ...
    def __init__(self, model: str,
        ...
        self.augment_unique = transforms.Compose([
            transforms.RandomApply([
                transforms.ColorJitter(0.4, 0.4, 0.4, 0.1)  # not strengthened
            ], p=0.8),
            transforms.RandomGrayscale(p=0.2),
        ...
        if self.model == "Sup":
            self.train_dataset = BimodalTrifeatures(root, split="train", transform=self.img_transform, **kwargs)
            self.val_dataset = BimodalTrifeatures(root, split="test", transform=self.img_transform, **kwargs)
        elif self.model == "FactorCL":
            self.train_dataset = BimodalTrifeaturesFactorCL(root, split="train", transform=self.img_transform,
                                                            augment=self.augment,
                                                            augment_unique=self.augment_unique, **kwargs)
            self.val_dataset = BimodalTrifeaturesFactorCL(root, split="test", transform=self.img_transform,
                                                          augment=self.augment,
                                                          augment_unique=self.augment_unique, **kwargs)
        else:
...
class BimodalTrifeaturesFactorCL(BimodalTrifeatures):
    def __init__(self, *args, augment, augment_unique, transform=None, **kwargs):
    ...
    def __getitem__(self, i):
        ...
        aug1 = self.augment(img1)
        aug2 = self.augment_unique(img2)
        if self.img_transform is not None:
            img1 = self.img_transform(img1)
            img2 = self.img_transform(img2)
        return img1, img2, aug1, aug2




if __name__ == "__main__":
    ds = BimodalTrifeatures("/fastdata/trifeatures_3combi", num_per_combination=3)
    print(ds[0])